...
/**
  * @brief Counter for sending sensor data over LORAWAN.
  * Sensor sleep time = 5 minutes, loopcounter = 2 => sending all 15 minutes
  * */
static int16_t loopcounter = 0;
...
static void SendTxData(void)
{
  ...
  uint32_t ticker_diff = 0;

  BME680_BSEC_MODULE_RUN();
  APP_LOG(TS_ON, VLEVEL_M, "BSEC Data done\r\n");
  APP_LOG(TS_ON, VLEVEL_M, "Timestamp to sleep: %d\r\n", time_stamp_interval_ms);
...
if (loopcounter == 2)
{
	...
	CayenneLppReset();
	CayenneLppAddBarometricPressure(channel++, (dataStorage.pressure/100));
	CayenneLppAddTemperature(channel++, dataStorage.temperature);
	CayenneLppAddRelativeHumidity(channel++, (dataStorage.humidity));
	CayenneLppAddAnalogInput(channel++, dataStorage.IAQAccuracy);
	CayenneLppAddAnalogInput(channel++, dataStorage.IAQ);
	CayenneLppAddLuminosity(channel++, dataStorage.CO2);
	CayenneLppAddAnalogInput(channel++, dataStorage.breath_voc_equivalent);
	CayenneLppAddAnalogInput(channel++, GetBatteryLevel());
...
else
{
	...
	APP_LOG(TS_ON, VLEVEL_L, "Loop number: %d\r\n", loopcounter);
	ticker_diff = HAL_GetTick()-ticker;
	APP_LOG(TS_ON, VLEVEL_L, "Tick difference: %d\r\n", ticker_diff);
